...
import com.tenext.auth.socket.JWebSocketClient
...
internal class WSClientManager private constructor() {
    ...
    private fun createSocketClient() {
        val myHost = if (debugTag.isNotEmpty())
            ...
            host
        //创建WebSocket
        client = JWebSocketClient(URI(myHost), handler)
        client!!.connect()
    }
     ...
     * 连接监听
     */
    private val connectListener = object : JWebSocketClient.ConnectListener {
        @Synchronized
